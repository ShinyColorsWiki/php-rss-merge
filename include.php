<?php
    class RSS {
        ...
        function __construct($id, $title, $link, $description, $feeds, $ttl = 3600, $encoding = 'UTF-8', $lang = 'en') {
            $this->id = $id;
            $this->title = $title;
            $this->link = $link;
    ...
    class InternalCache {
        ...
        function setCache(string $string) {
            global $cache;
            $item = $cache->getItem($this->id);
            $item->set($string);
    ...
    class Builder {
        ...
        function generateOutput(string $id) {
